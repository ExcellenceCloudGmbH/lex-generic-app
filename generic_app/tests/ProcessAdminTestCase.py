...
import inspect
from .TestExceptions import DetailedTestException

class ProcessAdminTestCase(TestCase):
    class TestDataNode:
        def __init__(self, path, object = None, nodes = []):
            self.path = path
            self.object = object
    ...
    class PathContext:
        ...
        def get_full_path(self):
            return "\n".join(["-" * (1 + index) + "> " + path for index, path in enumerate(self.path_stack)])
    ...
    def process_node(self, node, models):
        action = ""
        object = None
        self.path_context.push(node.path)
        try:
            for node in node.nodes:
                self.process_node(node, models)
                if node.isLeaf():
                    object = node.object
                    action = object['action']
                    klass = models[object['class']]
        ...
        except DetailedTestException as e:
            raise
        except Exception as e:
            raise DetailedTestException(
    ...
    #     self.tagged_objects = {}
    #     test_data, clean_test_path = self.get_test_data()
    #
    #
    ...
    def setUp(self) -> None:
        ...
        self.process_node(test_data, generic_app_models)
    ...
    def get_test_data(self):
        ...
        test_data = self.TestDataNode(clean_test_path, nodes=self.get_test_data_from_path(clean_test_path))
        return test_data
            ...
            with open(str(path), 'r') as f:
                ...
                for index, object in enumerate(test_data):
                    if "subprocess" in object:
                        self.path_context.push(path)  # Update path context
                        subprocess_path = object['subprocess'].replace('/', os.sep)
                        subprocess_path = os.getenv("PROJECT_ROOT") + os.sep + subprocess_path
                        sub_nodes = self.get_test_data_from_path(subprocess_path)
                        r.append(self.TestDataNode(path=subprocess_path, nodes=sub_nodes))
                        self.path_context.pop()  # Pop after subprocess is handled
                    else:
                        r.append(self.TestDataNode(path, object=object))
                        

            return r
        except json.JSONDecodeError as jde:
            print(f"JSON decoding error at position {jde.pos} in file {path}")
            print(f"Line: {jde.lineno}, Column: {jde.colno}, Message: {jde.msg}")
            print(f"Error occurred during: {self.path_context}")
            # traceback.print_exc()
            raise
        except Exception as e:
            print(f"Error occurred while processing test data from path: {path}")
            print(f"Error occurred during: {self.path_context}")
            print(f"Exception: {e}")
            # traceback.print_exc()
            raise

    def _handle_file_fields(self, tag, parameters):
        for parameter in parameters.keys():
            if isinstance(self.tagged_objects[tag]._meta.get_field(parameter), FileField):
                ...
                path = f"{upload_to}{os.path.basename(self.tagged_objects[tag].__dict__[parameter])}"
                file_name = os.path.basename(self.tagged_objects[tag].__dict__[parameter])
                with open(f"{os.getcwd()}/{self.tagged_objects[tag].__dict__[parameter]}", "rb") as f:
                    file_content = f.read()
                new_file_name = default_storage.save(path, content=File(io.BytesIO(file_content), name=f"{file_name}"))
                self.tagged_objects[tag].__dict__[parameter] = new_file_name

    def _update_file_field(self, tag, key, parameter):
        upload_to = self.tagged_objects[tag]._meta.get_field(key).upload_to
        if upload_to and not upload_to.endswith('/'):
            upload_to += "/"
        path = f"{upload_to}{os.path.basename(parameter)}"
        file_name = os.path.basename(parameter)
        with open(f"{os.getcwd()}/{parameter}", "rb") as f:
            file_content = f.read()
        new_file_name = default_storage.save(path, content=File(io.BytesIO(file_content), name=f"{file_name}"))
        self.tagged_objects[tag].__dict__[key] = new_file_name


    def get_classes(self, generic_app_models):
        test_data = self.get_test_data()
        return set([generic_app_models[node.object['class']] for node in test_data if node.object is not None])
...
class ProcessAdminTestCase2(TestCase):
