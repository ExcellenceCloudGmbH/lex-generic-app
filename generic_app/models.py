...
from generic_app.generic_models.upload_model import (
    UploadModelMixin,
...
def shorten_model_structure(current_model_struct):
    ...
    while len(current_model_struct.keys()) == 1 and not dict_is_leaf(
        get_first_dict_value(current_model_struct)
    ):
...
def is_special_file(file):
    return (
        file.stem.endswith("_test")
        or file.parents[0].name == "migrations"
        or file.parents[0].name == "tests"
        or file.stem.endswith("create_db")
        or file.stem.endswith("Log")
        or file.stem.endswith("Streamlit")
        or file.stem.endswith("CalculationIDs")
    )


def is_included_in_model_structure(file):
    return (
        not file.stem.endswith("model_structure")
        and not file.stem.endswith("authentication_settings")
        and not is_special_file(file)
    )
...
processAdminSite.register(
    [UserChangeLog, CalculationIDs, CalculationLog, Streamlit, Log]
)
        ...
        try:
            ...
            if name.endswith("model_structure"):
                ...
                if hasattr(imported_file, "get_global_filter_structure"):
                    global_filter_structure = (
                        imported_file.get_global_filter_structure()
                    )
                ...
                if issubclass(imported_class, HTMLReport):
                    ...
                    not issubclass(imported_class, type)
                    and imported_class._meta.abstract is not True
                ):
                    processAdminSite.register([imported_class])
                        ...
                        if os.getenv("CALLED_FROM_START_COMMAND"):
                            ...
                            def reset_instances_with_aborted_calculations():
                                if not os.getenv("CELERY_ACTIVE"):
                                    aborted_calc_instances = (
                                        imported_class.objects.filter(calculate=True)
                                    )
    ...
    for file in sorted_files:
        ...
        if is_included_in_model_structure(file):
            ...
            if issubclass(imported_class, HTMLReport):
                ...
                insert_model_to_structure(
                    model_structure, subfolders, imported_class._meta.model_name
                )

model_structure = shorten_model_structure(model_structure)
model_structure["Z_Reports"] = {
    "userchangelog": None,
    "calculationlog": None,
    "log": None,
}
